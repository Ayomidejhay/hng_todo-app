...
// // Utility to format due date nicely
...
//           className={`text-xs px-2 py-2 flex items-center justify-center rounded-full ${getPriorityStyles(todo.priority)}`}
//         >
//           {todo.priority}
//         </span>
//       </div>
...
//       <div>
//         <time data-testid="test-todo-due-date " className="text-gray-500">Due {formatDate(dueDate)}</time>
//         <time data-testid="test-todo-time-remaining" className="block text-xs text-gray-500">
//           {timeRemaining}
//         </time>
//       </div>
...
//           <span key={tag} data-testid={`test-todo-tag-${tag}`} className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-600">
//             {tag}
//           </span>
//         ))}
//       </div>
...
//         <button data-testid="test-todo-edit-button" onClick={() => onEdit(todo)} className="border border-gray-600 text-gray-600 px-2 py-1 rounded">
//           Edit
//         </button>
//         <button data-testid="test-todo-delete-button" onClick={() => onDelete(todo.id)} className="border px-2 py-1 rounded text-red-600">
//           Delete
//         </button>
//       </div>
//     </article>
...
//   if (diff < 0) return { text: `Overdue by ${Math.floor(Math.abs(diff) / (1000 * 60 * 60 * 24))} day(s) `, color: "text-red-600" };
//   if (days > 0) return { text: `Due in ${days} day(s)`, color: "text-green-600" };
//   if (hours > 0) return { text: `Due in ${hours} hour(s)`, color: "text-yellow-600" };
//   return { text: `Due in ${minutes} min`, color: "text-orange-600" };
// }
function getTimeMeta(dueDate: Date) {
  const now = new Date();
  const diff = dueDate.getTime() - now.getTime();
